...
Provider.prototype.mail = function (message, callback) {
	...
	var params;
	try {
		params = this._objectForMessage(message);
	} catch (error) {
		if (callback)
			callback(error);
	...
	this._ses.sendEmail(params, function (err) {
		if (!callback)
			return;
...
Provider.prototype._objectForMessage = function (message) {
	...
	if (message.replyto)
		obj.ReplyToAddresses = [message.replyto];

	if (message.textBody)
		obj.Message.Body.Text = { Data: message.textBody };

	if (message.htmlBody)
		obj.Message.Body.Html = { Data: message.htmlBody };
